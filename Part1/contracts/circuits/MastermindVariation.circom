...
template MastermindVariation() {
    ...
    // 1 (last iteration for duplication check) + 2 + 3 + 4 (first iteration for duplicate check) = 10
    component equalGuess[10]; 
    component equalSoln[10];
    var equalIdx = 0;

    // Create a constraint that the solution and guess digits are all less than 26 (because there are 26 alphabets).
    for (j=0; j<5; j++) {
